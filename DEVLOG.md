...
Entries are chronological, newest at the bottom. One entry per experiment or decision, not per day.

## Entry template

Each entry has five fields, a line or two each.

- **Changed:** the one variable, or "baseline" if nothing
- **Ran:** what, how many runs, model and key constants
- **Saw:** the result, with numbers
- **Means:** what we think it tells us, marked "unconfirmed" until it repeats
- **Next:** the next single-variable test

Rules for the log:

- One variable per experiment. If two things changed, that's two entries or a bad experiment.
- Numbers over adjectives. "3 of 5 died by T14" beats "most died early."
- Write the boring result too. A null result is a result.
...
No survivor. Bram, who lived through run 4, starved at T23 here. Ada got to T33 on a 1:1 work-to-buy ratio, which is nearly right (see the correction below); one buy too many and she went broke slowly instead of fast. Dot flipped sides: first time a worker persona died broke instead of starving.
